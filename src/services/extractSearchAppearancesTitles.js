/**
 * Extracts "Top job titles of your searchers" from search appearances HTML.
 * Logic mirrors parse-titles.mjs: find section by heading, collect next siblings
 * until H1–H6, parse each element for single percentage + title, dedupe by title|percentage.
 */

const extractSearchAppearancesTitles = ($) => {
    const sectionHeadingText = 'Top job titles of your searchers';
    ...
    return {
        topSearcherTitles: Array.from(resultsMap.values())
    };
};

module.exports = { extractSearchAppearancesTitles };
